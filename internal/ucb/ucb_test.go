...
func TestUCB(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			explorationRatio: 1,
			expected:         "A",
			description:      "Should select candidate with highest UCB score",
		},
		{
			name:             "Zero rounds",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := _UCB(tt.explorationRatio, tt.rewards, tt.chosen, tt.totalRounds)
			if result != tt.expected {
				t.Errorf("UCB() = %v, want %v", result, tt.expected)
...
func TestUCBExplorationExploitation(t *testing.T) {
	...
	result := _UCB(1, rewards, map[string]int{"exploited": 19, "explored": 1}, 20)
	if result != "explored" {
		t.Errorf("Expected exploration at low rounds, got %s", result)
	}

	// At high rounds, exploitation term should dominate
	result = _UCB(1, rewards, map[string]int{"exploited": 500, "explored": 500}, 1000)
	if result != "exploited" {
		t.Errorf("Expected exploitation at high rounds, got %s", result)
	}
}

func TestUCBMathematicalProperties(t *testing.T) {
	rewards := map[string]float64{"A": 10.0, "B": 5.0, "C": 8.0}
	chosen := map[string]int{"A": 10, "B": 10, "C": 10}
	totalRounds := 30

	// Test that the function is deterministic
	result1 := _UCB(1, rewards, chosen, totalRounds)
	result2 := _UCB(1, rewards, chosen, totalRounds)
	if result1 != result2 {
		t.Errorf("UCB should be deterministic, got %s and %s", result1, result2)
	}
}

func TestUCBEdgeCases(t *testing.T) {
	...
	t.Run("Large numbers", func(t *testing.T) {
		...
		result := _UCB(1, candidates, map[string]int{}, 1000)
		if result != "A" {
	...
	t.Run("Integer candidates", func(t *testing.T) {
		...
		result := _UCB(1, candidates, map[int]int{}, 10)
		if result != 1 {
...
func TestUCBConvergence(t *testing.T) {
	// Test that UCB converges to the best arm over time
	candidates := map[string]float64{
		"best":   10.0,
		"medium": 5.0,
		"worst":  1.0,
	...
	for round := 1; round <= 100000; round++ {
		result := _UCB(1, candidates, selections, round)
		selections[result]++
	}
...
func BenchmarkUCB(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_UCB(1, candidates, map[string]int{}, totalRounds)
	}
}
